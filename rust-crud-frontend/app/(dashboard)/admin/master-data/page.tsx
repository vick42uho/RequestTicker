...
import { RiSettings3Line, RiFolderSettingsLine, RiNodeTree, RiPriceTag3Line, RiBuildingLine } from "@remixicon/react";
...
export default function MasterDataAdminPage() {
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8 bg-background min-h-screen font-sans">
      {/* 🟢 Header Section */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-border">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-sm max-w-2xl">
            ตั้งค่าโครงสร้างแผนก (Departments), ประเภทบริการ (Types), หมวดหมู่ (Topics) และหัวข้อปัญหา (Subjects)
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="departments" className="space-y-6">
        <TabsList className="bg-muted p-1 rounded-xl w-full sm:w-auto overflow-x-auto flex justify-start">
          <TabsTrigger value="departments" className="flex items-center gap-2 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm px-4">
            <RiBuildingLine className="h-4 w-4" /> แผนก (Departments)
          </TabsTrigger>
        ...
        <TabsContent value="departments" className="focus-visible:outline-none"><DepartmentsTab /></TabsContent>
